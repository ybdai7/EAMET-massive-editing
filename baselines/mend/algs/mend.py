...
class MEND(EditableModel):
    ...
    def __init__(self, model, config, model_constructor, mend=None, edit_lrs=None):
        ...
        if not hasattr(self.model, "handles"):
            # hook the model for __x__ and __delta__ params
            # use the hooks to get the forward activations and backward
            # gradients, which correspond to original u,v
            hook_model(self.model, self.config.inner_params)
            LOG.info(f"Hooked {len(self.model.handles)//2} modules")

        if config.shared:
            shape_dict = defaultdict(list)
            # _inner_params gives a list of (name, param) tuples of all edited layers
            for n, p in _inner_params(
                model.named_parameters(), self.config.inner_params
            ):
                # shape_dict is the mapping from the layer shape to the list of edited layer names
                shape_dict[self.get_shape(p)].append(n)
            self.shape_dict = shape_dict

        if mend is None:
            # use ModuleDict, for different layer shapes, use different
            # GradientTransform params
            if not config.shared:
                self.mend = nn.ModuleDict(
                    {
                        # why replace "." with "#" here? -> to differentiate the
                        # default layer names
                ...
                # if shared params, for layers with the same shape, use the same GradientTransform
                self.mend = nn.ModuleDict(
                    {
                        str(tuple(s)): GradientTransform(
                            *s, config, len(shape_dict[s])
                        )
                        for s in shape_dict.keys() # here keys are the layer shapes
                    }
                )
    ...
    def state_dict(self, destination=None, prefix="", keep_vars=False):
        # get all state dicts, including the mend params
        # then remove the default model params through removing the keys

        # self.model.state_dict() -> default model params
        # super().state_dict() -> all params (default model params + mend params)
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        if self.config.shared:
            ...
            # __x__ for hooked input before certain layer
            # __delta__ for hooked backward gradient after certain layer
            transformed_factors = {
        ...
        # its for computing the outer product of x and delta
        # corresponding to the formula in the paper UV
        ...
        # here x, delta are already transformed by the mend parameters
        ...
        # using lr to scale the transformed grad
        ...
        # update the params with the scaled grad (scaled by lr)
        new_params = []
