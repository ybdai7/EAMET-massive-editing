...
def main(
    ...
    if model_path is not None:
        model = AutoModelForCausalLM.from_pretrained(model_path, cache_dir="your_cache_dir").cuda()
        if load_ori:
            model.config._name_or_path=model_name
        tok = AutoTokenizer.from_pretrained(model_path, cache_dir="your_cache_dir", add_bos_token=False)
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir="your_cache_dir").cuda()
        tok = AutoTokenizer.from_pretrained(model_name, cache_dir="your_cache_dir")
    ...
    ds = ds_class(DATA_DIR, tok=tok, size=dataset_size_limit, trigger = data_name, model_name_config = model_name_config, 
                  randomize_editing_sequence=randomize_editing_sequence, shuffle_seed=shuffle_seed)
    ...
    #####################################################
        ...
        elif alg_name=="FT" or alg_name=="MEND" or alg_name=="WISE" or alg_name=="GRACE":
        ...
        if "counterfact" in data_name:
            ...
            elif alg_name == "MEND" or alg_name == "WISE" or alg_name == "GRACE":
                overall_record = {
                    "alg_name": alg_name,
                    "rewrite_prompts_correct": all_rewrite_correct,
                    "paraphrase_prompts_correct": all_paraphrase_correct,
        ...
        if alg_name != "WISE" and alg_name != "GRACE":
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["EAMET", "MEMIT", "PMET", "ROME", "FT", "MEND", "ALPHAEDIT"],
        default="EAMET",
        help="Editing algorithm to use. Results are saved in results/<alg_name>/<run_id>, "
        "where a new run_id is generated on each run. "
        "If continuing from previous run, specify the run_id in --continue_from_run.",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="to store case metrics",
    )
    parser.add_argument(
        "--randomize_editing_sequence",
        dest="randomize_editing_sequence",
        action="store_true",
        help="to randomize editing sequence",
    )
    parser.add_argument(
        "--shuffle_seed",
        type=int,
        default=0,
        help="to shuffle seed",
    )
