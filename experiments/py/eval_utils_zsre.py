...
def test_batch_prediction_acc(model, tok, prompts: typing.List[str], target):
    ...
    if "deepseek" in str(name_or_path).lower():
        ...
        target_tok = tok(f" {target}")["input_ids"]
        if "llama-3.1" in str(name_or_path).lower() or \
...
def test_batch_prediction_acc_neighborhood(model, tok, prompts: typing.List[str], target):
    ...
    for i in range(0, len(prompts), samples_per_gpu):
        ...
        with torch.no_grad():
            ...
            if "mistral" in str(name_or_path).lower() \
                or "qwen" in str(name_or_path).lower() \
                or "deepseek" in str(name_or_path).lower():
                batch_correct_id = batch_correct_id
            elif "llama-3.1" in str(name_or_path).lower() or \
                "gemma" in str(name_or_path).lower():
